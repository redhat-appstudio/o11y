...
# Execute the command. If COMMAND_TO_RUN contains arguments,
# not quoting it allows those arguments to be passed correctly.
# This matches the behavior of $(CMD) in the Makefile.
${COMMAND_TO_RUN} -d "/work/${TEMP_DIR_BASENAME}/rules/" -y -p --tests-dir "/work/${TEMP_DIR_BASENAME}/tests/"

# The cleanup is now handled by the trap EXIT signal.
# The explicit rm -rf command from the original Makefile is no longer needed here.
