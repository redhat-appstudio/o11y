...
func TestGetTestErrorCounts(t *testing.T) {
	...
	cluster := "test-cluster"
	// Use the container data query for testing
	dataQuery := testTypes[0].DataQuery
...
func TestCheckSufficientTests(t *testing.T) {
	...
	cluster := "test-cluster"
	// Use the container row count query for testing
	rowCountQuery := testTypes[0].RowCountQuery
...
func TestCheckLatestTestIsRecent(t *testing.T) {
	...
	cluster := "test-cluster"
	// Use the container delay check query for testing
	delayCheckQuery := testTypes[0].DelayCheckQuery
