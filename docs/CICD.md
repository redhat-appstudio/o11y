...
Triggers on pushes to `main`. Runs the build and security chain **only**. Produces the signed production container image for exporters (see [EXPORTERS.md](EXPORTERS.md)).
...
- **Release engineering** (`config/stone-prd-rh01.pg1f.p1/service/ReleasePlanAdmission/rhtap-o11y/`) -- release engineering's acceptance of the release plan. `o11y.yaml` defines how the release happens: pushes the image to `quay.io/redhat-appstudio/o11y`, tags it with git SHA and `latest`, and runs an `infra-deployment-update-script` that auto-updates image references in [infra-deployments](https://github.com/redhat-appstudio/infra-deployments). The script only updates Grafana development and staging kustomization files (`components/monitoring/grafana/{development,staging}/`). Exporter deployments (registry, kanary) and Grafana production are updated separately via manual PRs to infra-deployments. This script is being removed ([PVO11Y-5227](https://redhat.atlassian.net/browse/PVO11Y-5227)) -- after that, all infra-deployments updates will be manual.
